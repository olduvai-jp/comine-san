...
> Note: ComfyUI native primitives (category `utils/primitive`) use `value` as the input key (e.g. `--Seed.value 123`).
...
- Native primitives (ComfyUI `utils/primitive`, input key: `value`)
  - **Input** `PrimitiveString`
  - **Input** `PrimitiveStringMultiline`
  - **Input** `PrimitiveInt`
  - **Input** `PrimitiveFloat`
  - **Input** `PrimitiveBoolean`
